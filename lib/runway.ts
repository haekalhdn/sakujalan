...
export function day(s: unknown): number {
  if (typeof s !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(s)) throw Error('Tanggal harus diisi dengan benar.');
  const n = Date.parse(s + 'T00:00:00Z');
  if (!Number.isFinite(n) || new Date(n).toISOString().slice(0,10) !== s) throw Error('Tanggal tidak valid.');
  return n / 86400000;
}
export const plus = (d: string, n: number) => new Date((day(d)+n)*86400000).toISOString().slice(0,10);
function amount(v: unknown, zero = true): number { if (!Number.isSafeInteger(v) || (v as number) < (zero ? 0 : 1) || (v as number) > 100000000) throw Error('Nominal harus rupiah utuh, maksimal Rp100 juta dan tidak negatif.'); return v as number; }
function title(v: unknown): string { if (typeof v !== 'string' || !v.trim() || v.trim().length > 100) throw Error('Isi keterangan sepanjang 1–100 karakter.'); return v.trim(); }
function bool(v: unknown): boolean { if (typeof v !== 'boolean') throw Error('Pilihan belum valid.'); return v; }
function find<T extends {id:string}>(items:T[], id:unknown):T { const x=items.find(i=>i.id===id); if (!x) throw Error('Catatan tidak ditemukan. Muat ulang halaman.'); return x; }
...
export function apply(input:State,c:Command,date=today(),id=crypto.randomUUID()):State {
  ...
  if(c.type==='budget') {
    if(c.consent!==true) throw Error('Setujui penyimpanan data terlebih dahulu.');
    const next=String(c.next); const h=day(next)-day(date); if(h<1||h>90)throw Error('Pilih tanggal uang saku 1–90 hari dari hari ini.');
    const daily=amount(c.daily), essentialDaily=amount(c.essentialDaily); if(essentialDaily>daily)throw Error('Kebutuhan minimum harian tidak boleh melebihi rencana belanja harian.');
    ...
    if(!s.budget)throw Error('Buat anggaran terlebih dahulu.');
    const b=s.budget;
    switch(c.type) {
      case 'plan': {
        const due=String(c.due); if(day(due)<day(date)||day(due)>day(date)+180)throw Error('Jadwal kebutuhan harus hari ini sampai 180 hari ke depan.');
      ...
      case 'save': {
        const p=find(s.plans,c.id); if(p.paid)throw Error('Kebutuhan sudah dibayar.');
        const v=amount(c.amount,false); if(v>=p.amount)throw Error('Biaya alternatif harus lebih kecil dari biaya rencana saat ini.'); p.amount=v; break;
      }
      case 'undoSave': { const p=find(s.plans,c.id); if(p.paid)throw Error('Kebutuhan sudah dibayar.'); p.amount=p.original; break; }
      case 'defer': {
        const p=find(s.plans,c.id); if(p.essential||p.paid)throw Error('Hanya kebutuhan opsional yang belum dibayar yang dapat ditunda.');
        const due=String(c.due); if(day(due)<day(b.next)||day(due)>day(date)+180||due<=p.due)throw Error('Pilih tanggal setelah jadwal lama, mulai periode berikutnya, maksimal 180 hari.');
        p.deferredFrom=p.deferredFrom||p.due; p.due=due; break;
      }
      case 'undoDefer': {const p=find(s.plans,c.id); if(p.paid||!p.deferredFrom)throw Error('Tidak ada penundaan yang bisa dibatalkan.');p.due=p.deferredFrom;delete p.deferredFrom;break;}
      case 'pay': { const p=find(s.plans,c.id); if(p.paid)throw Error('Kebutuhan sudah dibayar.'); const value=amount(c.amount,false); add('payment',value,p.title,{planId:p.id});p.paid=true;break; }
      case 'expense': case 'daily': case 'income': case 'transfer': {add(c.type,amount(c.amount,false),title(c.title));break;}
      case 'pending': {
        const due=String(c.due); if(day(due)<day(date)||day(due)>day(date)+180)throw Error('Pilih perkiraan penerimaan hari ini sampai 180 hari.');
        s.incomes.push({id,title:title(c.title),amount:amount(c.amount,false),due,status:'waiting'});break;
      }
      case 'receive': {const i=find(s.incomes,c.id); if(i.status==='received')throw Error('Pemasukan ini sudah diterima.'); add('income',amount(c.amount,false),i.title,{incomeId:i.id});i.status='received';break;}
      case 'refund': {
        const e=find(s.ledger,c.id); if(!['daily','expense','payment'].includes(e.kind))throw Error('Refund hanya untuk pengeluaran.');
        const refunded=s.ledger.filter(x=>x.refundOf===e.id).reduce((n,x)=>n+x.amount,0);const v=amount(c.amount,false);
        if(refunded+v>e.amount)throw Error('Refund melebihi pengeluaran yang belum dikembalikan.');add('refund',v,'Pengembalian: '+e.note,{refundOf:e.id});break;
      }
      case 'check': {s.checks=s.checks.filter(x=>x.date!==date);s.checks.push({date,met:bool(c.met),emergency:bool(c.emergency)});break;}
      default:throw Error('Tindakan tidak dikenal.');
    }
  }
  const cash=balance(s);if(cash<0||cash>100000000)throw Error('Saldo hasil pencatatan harus antara Rp0 dan Rp100 juta. Periksa pemasukan atau nominalnya.');
  if(s.ledger.length>5000||s.plans.length>500||s.incomes.length>500)throw Error('Batas catatan versi awal tercapai. Ekspor data untuk ditinjau.');
